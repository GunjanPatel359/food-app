...
const UserSettings = ({setTheme,theme}) => {
  ...
  useEffect(()=>{
    const initiatePage=async()=>{
      try {
        const res=await axios.get(`${backend_url}/user/colors/all-the-colors`,{withCredentials:true})
  ...
  const changeColor=async()=>{
    try {
      const res=await axios.patch(`${backend_url}/user/colors/change-color`,{color:theme},{withCredentials:true})
      if(res.data.success){
