...
const CurrentlyAssignedHotel = () => {
    ...
    useEffect(() => {
        const initiatePage = async () => {
            try {
                const res = await axios.get(`${backend_url}/user/hotel/find-assigned-hotel`, { withCredentials: true })
                console.log(res)
                if (res.data.success) {
                ...
                <div className="border p-5 shadow shadow-color4">
                    {hotels.length > 0 ? (<>
                                    ...
                                    <div key={i}>
                                        <div className="bg-white min-w-[300px] w-[250px] border border-color1 rounded shadow-color0 overflow-hidden shadow-lg">
                                            <div className="relative w-full h-44 overflow-hidden shadow-md shadow-color0">
                                                <img className="w-full h-full object-cover transition-transform duration-500 ease-in-out transform hover:scale-110 rounded-ss rounded-se cursor-pointer" src={`${img_url}/${item.restaurantId.imgUrl}`} />
                                            </div>
                                            <div className="p-2 flex flex-col gap-1">
                                                ...
                                                <div className='text-right mt-2 mb-2 mr-2'>
                                                    <span className='text-white bg-color5 font-semibold p-4 py-2 rounded shadow shadow-color1 cursor-pointer hover:shadow-md hover:shadow-color1 transition-all' onClick={()=>navigate(`/user/${item.restaurantId._id}/${item._id}/user-table`)}>Visit</span>
                                                    </div>
                        ...
                        <div className="flex items-center bg-red-50 border border-red-300 rounded-lg p-3 text-sm text-red-500">
                            <LuAlertTriangle size={20} className='mr-2 text-red-500' />
