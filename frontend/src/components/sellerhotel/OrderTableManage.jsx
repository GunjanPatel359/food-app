import { Plus } from 'lucide-react'
import { useModal } from '../../customhooks/zusthook'
import { useEffect, useState } from 'react'
import { toast } from 'react-toastify'
import axios from 'axios'
import { backend_url } from '../../server'
import { useParams } from 'react-router-dom'
import { MdDeleteForever, MdEdit } from 'react-icons/md'

const OrderTableManage = () => {
  const params=useParams()
  const {hotelId}=params

  const { onOpen,reloadCmd } = useModal()
  const [orderTables,setOrderTables]=useState([])

  useEffect(()=>{
    const getOrderTables=async()=>{
      try {
        const response=await axios.get(`${backend_url}/order-table/${hotelId}/get-all-tables`,{withCredentials:true})
        if(response.data.success){
          setOrderTables(response.data.hotel.tableIds)
        }
      } catch (error) {
        toast.error(error.data.message)
      }
    }
    getOrderTables()
  },[hotelId,reloadCmd])
  return (
    <div className='m-5'>
      {
        <div className='m-5 mt-8'>
          <div className='text-rose-500 font-semibold text-2xl mb-4'>
            Manage Tables
          </div>
          <div className='flex gap-2'>
            <span
              className='bg-rose-50 p-2 py-2 transition-all text-rose-500 hover:opacity-80 cursor-pointer border border-rose-500 border-dashed rounded-full flex flex-row max-w-fit pr-5'
              onClick={() => onOpen('create-order-table')}
            >
              <span className='flex flex-row border border-rose-500 rounded-full mr-2 ml-1 border-dashed'>
                <Plus className='inline' />
              </span>{' '}
              Create Table
            </span>
            <span
              className='bg-rose-50 p-2 py-2 transition-all text-rose-500 hover:opacity-80 cursor-pointer border border-rose-500 border-dashed rounded-full flex flex-row max-w-fit pr-5'
              onClick={() => onOpen()}
            >
              <span className='flex flex-row border border-rose-500 rounded-full mr-2 ml-1 border-dashed'>
                <Plus className='inline' />
              </span>{' '}
              Create Tables
            </span>
          </div>
          <div className='bg-rose-50 border border-rose-500 p-4 rounded mt-4 border-dashed text-lg flex flex-col gap-y-2'>
            {
              orderTables.map((table, i) => (
                <div key={i} className='bg-white p-3 rounded text-rose-500 border border-dashed border-rose-500'>
                  <div className='flex justify-between'>
                  <div><span className='font-semibold'>Table Number:</span> {table.tableNumber}</div>
                  <div className='my-auto flex gap-2'>
              <span>
                <span className='border border-rose-500 bg-rose-500 flex p-[3px] rounded shadow cursor-pointer'>
                  <MdEdit className='bg-rose-500 text-white inline' size={18} />
                </span>
              </span>
              <span>
                <span className='border border-rose-500 bg-rose-500 flex p-[3px] rounded shadow cursor-pointer'>
                  <MdDeleteForever
                    className='bg-rose-500 text-white inline'
                    size={18}
                  />
                </span>
              </span>
            </div>
                  </div>
                  <div className='flex justify-between'>
                  <div><span className='font-semibold'>Table Description:</span> {table.tableDescription}</div>
                  <div><span className='font-semibold'>Seats:</span> {table.seats}</div>
                    </div>
                </div>
              ))
            }
          </div>
        </div>
      }
    </div>
  )
}

export default OrderTableManage
